    ...
    <div class="accordion-content comments" id="comments">
        <?php if (!isset($is_public) || !$is_public): ?>
            <div class="comment-sorting">
                <small>
                    <?php if ($editable): ?>
                        <?= $this->modal->medium('comment', t('Add a comment'), 'CommentController', 'create', ['task_id' => $task['id']]) ?>
                    <?php endif ?>
                    <?= $this->url->icon('sort', t('Change sorting'), 'CommentController', 'toggleSorting', ['task_id' => $task['id'], 'csrf_token' => $this->app->getToken()->getReusableCSRFToken()]) ?>
                    <?php if ($editable): ?>
