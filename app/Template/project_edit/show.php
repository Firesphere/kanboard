...
<?php endif ?>
<form method="post" action="<?= $this->url->href('ProjectEditController', 'update', ['project_id' => $project['id'], 'redirect' => 'edit']) ?>" autocomplete="off">
    <?= $this->form->csrf() ?>
    ...
    <fieldset>
        ...
        <?= $this->form->text('name', $values, $errors, ['required', 'autofocus', 'tabindex="1"']) ?>

        <?= $this->form->label(t('Email'), 'email') ?>
        <?= $this->form->email('email', $values, $errors, ['tabindex="2"', 'autocomplete="email"']) ?>
        ...
        <?= $this->form->text('identifier', $values, $errors, ['maxlength="50"', 'tabindex="3"']) ?>
        ...
        <?= $this->form->checkbox('per_swimlane_task_limits', t('Task limits apply to each swimlane individually'), 1, $project['per_swimlane_task_limits'] == 1, '', ['tabindex' => 5]) ?>

        <?= $this->form->label(t('Task limit'), 'task_limit') ?>
        <?= $this->form->number('task_limit', $values, $errors, ['tabindex' => 6, 'min="0"']) ?>
    </fieldset>
    ...
    <fieldset>
        ...
        <div class="form-inline">
            ...
            <?= $this->form->select('owner_id', $owners, $values, $errors, ['tabindex="7"']) ?>
        </div>
    </fieldset>
    ...
    <fieldset>
        ...
        <?= $this->form->date(t('Start date'), 'start_date', $values, $errors, ['tabindex="8"']) ?>
        <?= $this->form->date(t('End date'), 'end_date', $values, $errors, ['tabindex="9"']) ?>
    </fieldset>
    ...
    <fieldset>
        ...
        <?= $this->form->number('priority_default', $values, $errors, ['tabindex="10"']) ?>

        <?= $this->form->label(t('Lowest priority'), 'priority_start') ?>
        <?= $this->form->number('priority_start', $values, $errors, ['tabindex="11"']) ?>

        <?= $this->form->label(t('Highest priority'), 'priority_end') ?>
        <?= $this->form->number('priority_end', $values, $errors, ['tabindex="12"']) ?>
    </fieldset>

    <?= $this->modal->submitButtons(['tabindex' => 13]) ?>
</form>
