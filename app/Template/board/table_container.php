<div id="board-container"
    ...
    <table id="board" class="board-project-<?= $project['id'] ?>">
        ...
        <table id="board"
               ...
               data-save-url="<?= $this->url->href('BoardAjaxController', 'save', ['project_id' => $project['id'], 'csrf_token' => $this->app->getToken()->getReusableCSRFToken()]) ?>"
               data-reload-url="<?= $this->url->href('BoardAjaxController', 'reload', ['project_id' => $project['id'], 'csrf_token' => $this->app->getToken()->getReusableCSRFToken()]) ?>"
               data-check-url="<?= $this->url->href('BoardAjaxController', 'check', ['project_id' => $project['id'], 'timestamp' => time(), 'csrf_token' => $this->app->getToken()->getReusableCSRFToken()]) ?>"
            ...
            <?php foreach ($swimlanes as $index => $swimlane): ?>
                <?php if (!($swimlane['nb_tasks'] === 0 && isset($not_editable))): ?>
                    ...
                    <?php if ($index === 0 && $swimlane['nb_swimlanes'] > 1): ?>
                        ...
                        <?= $this->render('board/table_column_first', [
                            'swimlane'     => $swimlane,
