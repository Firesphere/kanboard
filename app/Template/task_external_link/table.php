    ...
    <table class="table-striped table-scrolling">
        ...
        <?php foreach ($links as $link): ?>
            <tr>
                <td>
                    <?php if ($this->user->hasProjectAccess('TaskExternalLinkController', 'edit', $task['project_id'])): ?>
                        <div class="dropdown">
                            ...
                            <ul>
                                <li>
                                    <?= $this->modal->medium('edit', t('Edit'), 'TaskExternalLinkController', 'edit', ['link_id' => $link['id'], 'task_id' => $task['id']]) ?>
                                </li>
                                <li>
                                    <?= $this->modal->confirm('trash-o', t('Remove'), 'TaskExternalLinkController', 'confirm', ['link_id' => $link['id'], 'task_id' => $task['id']]) ?>
                                </li>
                            </ul>
                        </div>
                    <?php endif ?>
                ...
                <td>
                    <a href="<?= $this->text->e($link['url']) ?>" title="<?= $this->text->e($link['url']) ?>"
                       target="_blank"><?= $this->text->e($link['title']) ?><span
                                class="ui-helper-hidden-accessible"> (<?= $this->text->e($link['url']) ?>)</span></a>
                </td>
                <td>
