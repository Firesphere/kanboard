...
<?php if ($project['is_public']): ?>
    ...
    <?= $this->url->link(t('Disable public access'), 'ProjectViewController', 'updateSharing', ['project_id' => $project['id'], 'switch' => 'disable'], true, 'btn btn-red') ?>
<?php else: ?>
    <p class="alert alert-info"><?= t('This feature enables the iCal feed, RSS feed and the public board view.') ?></p>
    <?= $this->url->link(t('Enable public access'), 'ProjectViewController', 'updateSharing', ['project_id' => $project['id'], 'switch' => 'enable'], true, 'btn btn-blue') ?>
<?php endif ?>
